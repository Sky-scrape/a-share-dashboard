...
def load_us_rows() -> dict:
    """隔夜美股因子 {A股T: 行}。单一来源：backend/us_market.py 构建的
    data/recap/us_market/factors.json（键=A股交易日，us_date=实际对齐的美股交易日，
    恒为在 T 日 09:15 北京时间前收盘的最近一场，无未来函数）。
    缺失返回 {}——US 因子诚实降级，C4+ 轮次由 run_round 断言覆盖率。"""
    p = BASE.parent / "data" / "recap" / "us_market" / "factors.json"
    try:
