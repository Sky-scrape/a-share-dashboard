...
def run_round(version: str, out_dir: Path, store=None, pre=None) -> dict:
    ...
    # US 因子覆盖率硬断言（C4+）：启用美股维度时，区间内隔夜纳指缺失日不得超过 5%
    cfg_us_on = ("us_night_force_defensive" in cfg["regime"]
                 or "us" in cfg.get("lu_group", {}).get("weights", {})
                 or "us" in cfg.get("nlu_group", {}).get("weights", {}))
    if cfg_us_on:
        have = sum(1 for d in tdays
                   if d in ms.index and not pd.isna(ms.loc[d, "us_ndx"]))
        cov = have / len(tdays) if tdays else 0.0
        if cov < 0.95:
            raise RuntimeError(
    ...
    for T in tdays:
        ...
        note = {"T": T, "regime": regime,
                ...
                "us_ndx": (float(mrow["us_ndx"])
                           if mrow is not None and "us_ndx" in mrow.index
                           and pd.notna(mrow["us_ndx"]) else None),
