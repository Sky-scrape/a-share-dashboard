...
系统按「数据基石 → 体验闭环 → 深度分析」三层堆叠。下层保证数据可信，中层让人盘中不被噪音打断，上层把盘面压成可落地的判断——**每层只解决一个问题，越往上越靠近结论**。

![A股看板三层架构总览](assets/docs/roadmap.jpg)

| 层 | 关注点 | 代表能力 |
| --- | --- | --- |
| **P0 数据基石** | 数据可不可信 | 数据健康监控（延迟/完整性/异常率/源数）、定时任务可观测性、缺数据兜底、自愈运维 |
| **P1 体验闭环** | 盘中顺不顺手 | 盘中自动跟踪、智能预警、复盘一键导出、自选分组 |
| **P2 深度分析** | 结论站不站得住 | 市场情绪指数（强弱分界 50）、板块轮动统计、策略备选池与样本外追踪 |
